...
clean_shoe_img <- function(im) {
  suppressMessages({
    ...
    crop.bbox(im, im_bbox) %>%
      ...
      autocrop() %>%
      grayscale() %>%
      pad(10, axes = "xy", val = max(.))
  })
}

imgs <- lapply(imlinks[9:16], function(.) load.image(.) %>% clean_shoe_img()) %>% as.imlist()
```
...
match_points <- purrr::map2(
  ...
  ~knn_points(.x, kpf[[1]], .y, hkp_centers[[1]], ratio = .80, show_plot = T))
```
